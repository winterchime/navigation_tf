...
int main(int argc, char** argv){
  ...
  ros::init(argc, argv, "ultra1_tf2_broadcaster");
  // create a node
  ros::NodeHandle node;

  // create the transform broadcaster (tfb)
  tf2_ros::TransformBroadcaster tfb;
  // create a new transform, from the parent robot_base to the new child ultra1_base
  geometry_msgs::TransformStamped transformStamped;

  // parent is robot_base
  transformStamped.header.frame_id = "robot_base";
  // child is ultra1_base
  transformStamped.child_frame_id = "ultra1_base";
  transformStamped.transform.translation.x = 4.0;
  // child frame ultra1_base is 4 meters offset from parent frame robot_base
